...
# Update from Fahim (5 July 23)

To update this repo further, we need to first change the `config/_default/config.yaml` with following:
...
outputs:
  home: []
```
Then run `hugo mod clean` command, then revert the changes to `config.yaml`.
